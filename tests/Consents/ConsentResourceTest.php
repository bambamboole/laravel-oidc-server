...
beforeEach(function (): void {
    ...
    // Both resources declare `read`: the same scope name means a different
    // thing at each of them, which is exactly what a consent has to tell apart.
...
// The bug: `read` at the orders API is not `read` at the billing API, but a
// consent that only knew the scope name covered both.
...
it('skips the screen for the resource the user did approve', function (): void {
    approveForResource($this, [ORDERS]);
...
// Without a `resource` the request is addressed to the realm itself, which is
// a resource identifier like any other.
