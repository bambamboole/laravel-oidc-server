...
it('hangs the realm off the origin when realms are routed by path', function (): void {
    config(['oidc.issuer' => 'https://id.example.com', 'oidc.routes.realms' => 'path', 'oidc.realm' => 'acme']);

    expect(app(RealmIssuerResolver::class)->url())->toBe('https://id.example.com/realms/acme');
});
