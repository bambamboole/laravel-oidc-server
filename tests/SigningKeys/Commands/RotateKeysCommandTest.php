...
{
    return array_map(fn (SigningKeyPair $key): string => $key->kid(), app(SigningKeys::class)->verificationKeys());
}

it('stores a new signing key and retires the current one for verification', function (): void {
    $currentKid = app(SigningKeys::class)->signingKid();

    $this->artisan('oidc:rotate-keys', ['--force' => true])
        ->expectsOutputToContain('stays in JWKS')
        ->assertSuccessful();

    expect(app(SigningKeys::class)->signingKid())->not->toBe($currentKid)
        ->and(signingKids())->toBe([app(SigningKeys::class)->signingKid(), $currentKid])
...
it('generates the first key without confirmation with --if-missing when none exists', function (): void {
    ...
    expect(SigningKey::query()->count())->toBe(1)
        ->and(app(SigningKeys::class)->signingKey()->privateKey())->toContain('PRIVATE KEY');
});
